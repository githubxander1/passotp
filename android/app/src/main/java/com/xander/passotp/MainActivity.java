...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b) { super.onCreate(b); prefs=getSharedPreferences("passotp",MODE_PRIVATE); otpOnly=prefs.getBoolean("otp_only",true); showUnlock(); }
    ...
    void showHome() {
        ...
        filter.setOnCheckedChangeListener((b,c)->{otpOnly=c; prefs.edit().putBoolean("otp_only",c).apply(); renderList();}); add.setOnClickListener(v->edit(null)); settings.setOnClickListener(v->showSettings()); autofill.setOnClickListener(v->openAutofillSettings()); lock.setOnClickListener(v->{key=null;entries.clear();prefs.edit().remove("session_key").remove("session_until").apply();showUnlock();});
    ...
    void openAutofillSettings(){try{startActivity(new Intent(Settings.ACTION_REQUEST_SET_AUTOFILL_SERVICE, Uri.parse("package:"+getPackageName())));}catch(Exception e){startActivity(new Intent(Settings.ACTION_SETTINGS));}}
    void showSettings(){
        Dialog dialog=new Dialog(this);LinearLayout panel=new LinearLayout(this);panel.setOrientation(LinearLayout.VERTICAL);panel.setPadding(dp(22),dp(18),dp(22),dp(18));TextView title=text("设置",22);title.setTypeface(null,1);panel.addView(title);TextView intro=text("管理显示、自动填充和本地数据",12);intro.setTextColor(Color.rgb(104,115,133));panel.addView(intro);
        TextView display=text("显示",14);display.setTypeface(null,1);display.setTextColor(Color.rgb(36,99,235));LinearLayout.LayoutParams section=new LinearLayout.LayoutParams(-1,dp(34));section.topMargin=dp(18);panel.addView(display,section);
        CheckBox otp=new CheckBox(this);otp.setText("首页默认只显示含 OTP 的项目");otp.setTextSize(14);otp.setChecked(otpOnly);panel.addView(otp,new LinearLayout.LayoutParams(-1,dp(48)));
        TextView security=text("安全与填充",14);security.setTypeface(null,1);security.setTextColor(Color.rgb(36,99,235));LinearLayout.LayoutParams securitySection=new LinearLayout.LayoutParams(-1,dp(34));securitySection.topMargin=dp(12);panel.addView(security,securitySection);
        Button autofill=softButton("启用系统自动填充");panel.addView(autofill,new LinearLayout.LayoutParams(-1,dp(46)));autofill.setOnClickListener(v->openAutofillSettings());
        Button biometric=softButton("生物识别解锁");panel.addView(biometric,new LinearLayout.LayoutParams(-1,dp(46)));biometric.setOnClickListener(v->Toast.makeText(this,"请先锁定应用，再从解锁页使用生物识别",Toast.LENGTH_LONG).show());
        TextView data=text("数据",14);data.setTypeface(null,1);data.setTextColor(Color.rgb(36,99,235));LinearLayout.LayoutParams dataSection=new LinearLayout.LayoutParams(-1,dp(34));dataSection.topMargin=dp(12);panel.addView(data,dataSection);
        Button dataButton=softButton("导入/导出加密保险库");panel.addView(dataButton,new LinearLayout.LayoutParams(-1,dp(46)));dataButton.setOnClickListener(v->{dialog.dismiss();showDataMenu();});
        TextView version=text("PassOTP Android 0.2.3",11);version.setTextColor(Color.rgb(104,115,133));version.setPadding(0,dp(16),0,dp(6));panel.addView(version);Button close=button("完成");LinearLayout.LayoutParams closeParams=new LinearLayout.LayoutParams(-1,dp(46));closeParams.topMargin=dp(8);panel.addView(close,closeParams);close.setOnClickListener(v->dialog.dismiss());
        dialog.setContentView(panel);otp.setOnCheckedChangeListener((b,c)->{otpOnly=c;prefs.edit().putBoolean("otp_only",c).apply();renderList();});dialog.setOnShowListener(v->sizeDialog(dialog));dialog.show();sizeDialog(dialog);
    }
