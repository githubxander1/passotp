...
async function createStandaloneConfigKey() { return crypto.subtle.generateKey({ name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"]); }
async function encryptStandaloneConfig(config) { const key = await createStandaloneConfigKey(); const record = await encryptWithKey(config, key); const raw = await crypto.subtle.exportKey("raw", key); return { key: bytesToB64(raw), record }; }
async function decryptStandaloneConfig(stored) { const key = await crypto.subtle.importKey("raw", b64ToBytes(stored.key), { name: "AES-GCM" }, true, ["encrypt", "decrypt"]); return decryptWithKey(stored.record, key); }
async function saveStandaloneWebdavConfig(config) { await chrome.storage.local.set({ webdavConfigStandalone: await encryptStandaloneConfig(config) }); }
async function saveWebdavConfig() { const config = { primary: readWebdavConfig(), secondary: readWebdavConfig("2") }; await saveStandaloneWebdavConfig(config); $("syncMessage").textContent = "两个 WebDAV 配置已独立加密保存"; }
async function loadWebdavConfig() { const stored = await chrome.storage.local.get(["webdavConfigStandalone", "webdavConfig"]); try { let config; if (stored.webdavConfigStandalone) config = await decryptStandaloneConfig(stored.webdavConfigStandalone); else if (stored.webdavConfig && state.key) { config = await decryptWithKey(stored.webdavConfig, state.key); await saveStandaloneWebdavConfig(config); } else return; if (config.url) { writeWebdavConfig("", config); return; } writeWebdavConfig("", config.primary); writeWebdavConfig("2", config.secondary); } catch { $("syncMessage").textContent = "WebDAV 配置记录损坏，请重新填写后保存。"; } }
async function migrateWebdavConfig(oldKey) { const stored = await chrome.storage.local.get(["webdavConfigStandalone", "webdavConfig"]); if (stored.webdavConfigStandalone || !stored.webdavConfig) return; const config = await decryptWithKey(stored.webdavConfig, oldKey); await saveStandaloneWebdavConfig(config); }
async function changeMasterPassword() { const current = $("currentMasterPassword").value, next = $("newMasterPassword").value, confirm = $("confirmMasterPassword").value, message = $("masterPasswordMessage"); message.textContent = ""; if (next.length < 8) { message.textContent = "新主密码至少 8 位"; return; } if (next !== confirm) { message.textContent = "两次新主密码不一致"; return; } try { const stored = await chrome.storage.local.get(["salt", "vault"]); const oldKey = await deriveKey(current, b64ToBytes(stored.salt)); await decryptWithKey(stored.vault, oldKey); const salt = crypto.getRandomValues(new Uint8Array(16)); const key = await deriveKey(next, salt); await migrateWebdavConfig(oldKey); const encrypted = await encryptWithKey(state.entries, key); await chrome.storage.local.set({ salt: bytesToB64(salt), vault: encrypted }); state.salt = salt; state.key = key; await saveSessionKey(); $("currentMasterPassword").value = ""; $("newMasterPassword").value = ""; $("confirmMasterPassword").value = ""; message.textContent = "主密码已修改，WebDAV 配置已独立保存"; } catch (error) { message.textContent = error.message?.includes("OperationError") ? "当前主密码不正确，或 WebDAV 配置无法迁移；主密码未修改" : "当前主密码不正确或保险库损坏"; } }
...
async function vaultPackage() {
  const stored = await chrome.storage.local.get(["salt", "vault", "webdavConfigStandalone", "webdavConfig"]);
  const packageData = { format: "local-password-otp-vault", version: 2, exportedAt: new Date().toISOString(), salt: stored.salt, vault: stored.vault };
  let webdavConfig = null;
  if (stored.webdavConfigStandalone) webdavConfig = await decryptStandaloneConfig(stored.webdavConfigStandalone);
  else if (stored.webdavConfig && state.key) webdavConfig = await decryptWithKey(stored.webdavConfig, state.key);
  if (webdavConfig) packageData.webdavConfig = await encryptWithKey(webdavConfig, state.key);
  return packageData;
}
async function exportVault() { const blob = new Blob([JSON.stringify(await vaultPackage(), null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const filename = `password-otp-vault-${new Date().toISOString().slice(0, 10)}.json`; if (chrome.downloads?.download) chrome.downloads.download({ url, filename, saveAs: true }, () => setTimeout(() => URL.revokeObjectURL(url), 10000)); else { const link = document.createElement("a"); link.href = url; link.download = filename; link.click(); setTimeout(() => URL.revokeObjectURL(url), 10000); } }
async function importVault(file) { const data = JSON.parse(await file.text()); if (data.encrypted === true) throw new Error("Bitwarden 加密导出需要先在 Bitwarden 中导出为未加密 JSON"); if (Array.isArray(data.items) && data.encrypted === false) { if (!state.key) throw new Error("请先解锁后再导入 Bitwarden 数据"); let added = 0; for (const item of data.items) { if (item.type !== 1 || !item.login) continue; const rawOtp = String(item.login.totp || ""); let otp = "", algorithm = "SHA1", digits = 6, period = 30; if (rawOtp) { try { const parsed = rawOtp.toLowerCase().startsWith("otpauth://") ? parseOtpInput(rawOtp) : { secret: normalizeBase32(rawOtp) }; otp = parsed.secret; algorithm = parsed.algorithm || algorithm; digits = parsed.digits || digits; period = parsed.period || period; } catch {} } const entry = { id: crypto.randomUUID(), name: item.name || "未命名", username: item.login.username || "", password: item.login.password || "", notes: item.notes || "", otp, algorithm, digits, period }; if (!state.entries.some((existing) => existing.name === entry.name && existing.username === entry.username && existing.password === entry.password)) { state.entries.unshift(entry); added++; } } await saveVault(); closeSettings(); render(); alert(`Bitwarden 导入完成：新增 ${added} 项`); return; } if (data.format !== "local-password-otp-vault" || !data.salt || !data.vault) throw new Error("不是本扩展的有效导出文件或 Bitwarden 未加密 JSON"); const nextStorage = { salt: data.salt, vault: data.vault }; if (data.webdavConfig) nextStorage.webdavConfig = data.webdavConfig; await chrome.storage.local.set(nextStorage); if (data.webdavConfig) await chrome.storage.local.remove("webdavConfigStandalone"); await chrome.storage.session.remove("sessionKey"); lock(); alert(data.webdavConfig ? "导入成功，WebDAV 配置已恢复，请使用导出时的主密码解锁" : "导入成功，请使用导出时的主密码解锁"); }
...
$("scanTabButton")?.addEventListener("click", () => scanCurrentTab().catch((error) => $("scanMessage").textContent = error.message)); $("scanRegionButton")?.addEventListener("click", () => startTabRegionScanner().catch((error) => $("scanMessage").textContent = error.message)); $("scanRegionConfirm")?.addEventListener("click", () => scanSelectedRegion().catch((error) => $("scanMessage").textContent = error.message)); $("scanRegionCancel")?.addEventListener("click", closeRegionScanner); if ($("scanCanvas")) bindCropCanvas(); $("scanCameraButton")?.addEventListener("click", () => startScanner().catch((error) => $("scanMessage").textContent = error.message)); $("stopScanButton")?.addEventListener("click", () => { stopScanner(); $("scanVideo")?.classList.add("hidden"); $("stopScanButton")?.classList.add("hidden"); }); $("scanFile")?.addEventListener("change", (e) => scanImage(e.target.files[0]).catch((error) => $("scanMessage").textContent = error.message));
